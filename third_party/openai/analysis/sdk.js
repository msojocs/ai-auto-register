var SentinelSDK=function(t){
    ...
    function C(t,n,r){
        ...
        const c=(t=t||{}).random??t.rng?.()??function(){
            const t=l;
            if(!a){
                if("undefined"==typeof crypto||!crypto.getRandomValues)throw new Error(t(2));a=crypto[t(0)].bind(crypto)
            }
            return a(d)
        }();
        if(c[o(7)]<16)
            throw new Error(o(6));
        return c[6]=15&c[6]|64,c[8]=63&c[8]|128,function(t,n=0){const r=e;return(i[t[n+0]]+i[t[n+1]]+i[t[n+2]]+i[t[n+3]]+"-"+i[t[n+4]]+i[t[n+5]]+"-"+i[t[n+6]]+i[t[n+7]]+"-"+i[t[n+8]]+i[t[n+9]]+"-"+i[t[n+10]]+i[t[n+11]]+i[t[n+12]]+i[t[n+13]]+i[t[n+14]]+i[t[n+15]])[r(5)]()}(c)
    }
    b();
const A=j;
...
class _{
    ...
    async function we(t){
        ...
        console.info('[init] P.getRequirementsToken', P.getRequirementsToken)
        console.info('[init] token:', token)
        const result = await(async function(t,n){
        ...
        console.info('[init] token result:', result)
        return result
    }
    async function ye(name){
        console.info('[token] get token for:', name)
        if(Yn)
        ...
        try{
            ...
            // ce 补充id(cookies[oai-did])和flow(name)字段
            o=ce({
                p: r,
            ...
            const result = (e['cachedChatReq']=null,setTimeout((async()=>{
    ...
    return Yn?function(){
        window['addEventListener']('message',(async n=>{if(n['source']===window)return;
    // console.info('e(33):', e(33));
    // console.info('n[e(33)]:', n[e(33)]);
    const{type:r,flow:o,requestId:i,p:c}=n['data']??{};
    if("init"!==r&&r!=='token')
        return;const s="string"==typeof o&&o['length']>0?o:te;
    try{
        console.trace("p:", c);
        let t;
        (r === 'init'
          ...
          : "token" === r &&
            (t = await (async function (t, n) {
              ...
              if (
                !r["cachedChatReq"] ||
                o - r["lastFetchTime"] > 54e4 ||
                r["cachedProof"] !== n
              ) {
                const r = await Promise["race"]([
                  ue(t, n),
                  new Promise((r) =>
                    setTimeout(() => r(ce({ e: "elapsed", p: n }, t)), 4e3),
                  ),
                ]);
          ...
          n['source']?.['postMessage'](
            { type: 'response', requestId: i, result: t },
            { targetOrigin: n.origin },
          ));
    }
    catch(t){
        n['source']?.['postMessage']({type:"response",requestId:i,error:t['message']},{targetOrigin:n['origin']})
    }
}))
