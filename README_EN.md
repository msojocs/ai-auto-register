...
| Storage | GORM + SQLite (migrates to PostgreSQL) |
...
On first startup with an empty database, the server creates a default admin account using the values in [server/config/config.yaml](server/config/config.yaml) or the `DEFAULT_ADMIN_USERNAME` / `DEFAULT_ADMIN_PASSWORD` environment variables. The defaults are `admin` / `admin123456`.

SQLite uses `github.com/mattn/go-sqlite3` which requires CGO to be enabled.
...
│   │   ├── core/           # Worker Pool
│   │   ├── executor/       # Platform plugins (chatgpt, cursor, etc.)
...
| `DB_TIMEZONE` | `Local` | Database time zone |
